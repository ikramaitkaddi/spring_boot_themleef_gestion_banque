...
public class OperationMetierImpl implements OperationMetier {
	...
	public PageOperation getOperation(String codeCompte, int page, int size) {
	   ...
	   //System.out.println("in getOperation" +codeCompte);
	//Page<Operation> ops= operationRepository.findBycodeCompte(codeCompte, PageRequest.of(page, size));
		
	// 2 éme Solution
	  Compte cp = compteRepository.findById(codeCompte).orElse(null);
	Page<Operation> ops2 = operationRepository.findByCompte(cp,PageRequest.of(page, size));
